...
* **Lõi xử lý AI (Hybrid Core):**
  * **SAM 2 / SAM 3 (Segment Anything Model):** Phụ trách việc định hình hình dáng và bám đuổi vật thể (Tracking/Propagation) qua các khung hình dựa trên điểm click chuột.
...
* **FR-06 (Quản lý Model AI - Model Hub):** Ứng dụng không nhúng sẵn model đồ sộ để tránh làm file cài đặt quá nặng. Hệ thống tự động nhận diện phần cứng (RAM, VRAM) để đề xuất và cho phép người dùng tải xuống phiên bản model (Tiny, Small, Large) phù hợp với cấu hình.
...
|  - Hardware Profiler & Model Manager: Quản lý & tải Checkpoint  |
|  - OpenCV & FFmpeg: Rã video thành Frame chuỗi, quản lý bộ đệm  |
|  - SAM 2 / SAM 3 Predictor: Tiếp nhận Click -> Tạo & Đuổi Mask  |
...
1. **Khởi tạo & Tải Model:** Ứng dụng quét cấu hình máy. Dựa vào thiết lập, Model Manager tải checkpoint (SAM 2/SAM 3/MatAnyone) tương ứng (nếu chưa có) và khởi tạo Model qua cơ chế Dynamic Factory.
2. **Trích xuất Video thành Frame:** Khi Import Video, Backend dùng OpenCV bóc tách video thành các file ảnh Frame riêng lẻ lưu vào thư mục Cache tạm thời trên SSD, giúp tối ưu hóa tốc độ đọc ngẫu nhiên (Random Access) khi người dùng kéo Timeline qua lại.
3. **Tạo Embedding & Nhận diện Vật thể:** Khi dừng ở 1 frame, SAM Image Encoder mã hóa frame đó thành dạng Feature Embedding. Khi người dùng click chuột, tọa độ được gửi xuống Backend để SAM Decoder tính toán ra vùng chứa vật thể (Binary Mask) ngay lập tức.
4. **Lan truyền Mặt nạ (Propagation):** Người dùng bấm nút "Track". Trình `Video Predictor` của SAM sẽ kích hoạt, truyền trạng thái bộ nhớ (Memory Bank) từ frame hiện tại sang các frame kế tiếp để tự động giữ mặt nạ bám theo chuyển động của vật thể.
5. **Làm mịn và Tách viền (Matting):** Các chuỗi Mặt nạ thô từ SAM được đẩy qua mô hình **MatAnyone 2**. Tại đây, AI sẽ đối chiếu ảnh gốc với mặt nạ thô để tính toán giá trị độ mờ (Alpha values) cho vùng rìa, tách chính xác các chi tiết siêu nhỏ (tóc bay, chuyển động nhòe).
6. **Đóng gói Sản phẩm Đầu ra:** Mặt nạ hoàn chỉnh sau tinh chỉnh được FFmpeg đọc lên, áp thành kênh Alpha đè lên video gốc để xuất ra định dạng trong suốt `.mov` (ProRes 4444) hoặc chuỗi ảnh Luma Matte theo yêu cầu của Editor.
...
```
+-----------------------------------------------------------------------+
|  Menu: Dự Án | Model Hub (Quản lý tải Checkpoint) | Trợ Giúp           |
+-----------------------------------+-----------------------------------+
|                                   |  [BẢNG CÔNG CỤ AI]                |
...
* **AI Models:**
  * **SAM 2 / SAM 3:** Nhân cốt lõi để nhận diện vật thể và tracking qua các frame. Hệ thống sử dụng Factory Pattern để tải động các biến thể (Tiny, Small, Base, Large) tương ứng với tài nguyên máy tính.
  * **MatAnyone 2:** Tích hợp quy trình xử lý Matting, nhận đầu vào là Trimap/Mask thô để tạo ra lớp Alpha Matte siêu thực.
* **Hardware Profiler & Model Management:** Sử dụng thư viện `torch.cuda` / `psutil` để quét phần cứng, quản lý tải checkpoint qua JSON Registry.
* **Xử lý Đồ họa & Video:** 
...
* **Công việc:**
  ...
  * Xây dựng Model Hub: Hiện Popup đo lường sức mạnh GPU ở lần chạy đầu, gợi ý bộ Checkpoint (SAM 2/SAM 3) phù hợp và tải xuống hiển thị thanh tiến trình (Progress bar).
...
| :--- | :---: | :--- |
| **Tràn VRAM (Out of Memory) trên GPU yếu** | Cao | Cung cấp tùy chọn cấu hình AI: Thông qua Model Hub, hệ thống tự block các bản model Large và khuyến khích người dùng tải bản `Tiny/Small`. Hoặc hạ độ phân giải tính toán (Inference size) xuống $480p/720p$. |
...
| **Lỗi mất Tracking (Vật thể bị che khuất hoặc rời khỏi khung hình)** | Trung bình | Phát triển cơ chế "Smart Timeline": Cảnh báo người dùng khi độ tin cậy (Confidence Score) của SAM tụt xuống dưới ngưỡng an toàn, tự động tạm dừng AI để người dùng can thiệp bằng tay. |
